...
if (!window.__navigationFixInitialized) {
  ...
  function isHomePage(path) {
    return (
      path.endsWith('/vscode-f5xc-tools/') ||
      path.endsWith('/vscode-f5xc-tools/index.html') ||
      path === '/'
    );
  }
  ...
  function fixActiveTab() {
    ...
    tabItems.forEach(function (item) {
      ...
      var isHome = tabPath === '/vscode-f5xc-tools/' || tabPath.endsWith('/vscode-f5xc-tools/');
      var pathMatches = isHome ? isHomePage(path) : path.startsWith(tabPath) && !isHomePage(path);
